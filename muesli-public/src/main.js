const { app, BrowserWindow, ipcMain, protocol, Notification, dialog, shell } = require('electron');
...
app.whenReady().then(() => {
  // Set app ID for notifications (important for macOS)
  if (process.platform === 'darwin') {
    app.setAppUserModelId('com.muesli.app');
  }
...
function initSDK() {
  ...
  RecallAiSdk.addEventListener('meeting-detected', (evt) => {
    ...
    // Create and show notification with single action button
    const notification = new Notification({
      title: 'Meeting detected',
      body: platformName, // Keep under 256 bytes for macOS
      silent: false,
      urgency: 'critical', // Make notification more prominent
      sound: 'default', // Add sound to grab attention
      actions: [
        {
          type: 'button',
          text: 'Take Notes'
    ...
    notification.on('click', () => {
      console.log("Notification body clicked for platform:", platformName);
      joinDetectedMeeting();
    ...
    notification.on('action', (event, index) => {
      console.log(`Notification action clicked: index ${index}`);
      if (index === 0) {
        // "Take Notes" button clicked
        console.log("Take Notes button clicked - starting recording");
        joinDetectedMeeting();
    ...
    notification.on('show', () => {
      console.log("OS Notification displayed with action buttons");
    });

    notification.on('failed', (event, error) => {
      console.error("Notification failed to show:", error);
      // Fallback to focusing the app window
      if (mainWindow) {
        if (mainWindow.isMinimized()) mainWindow.restore();
        mainWindow.focus();
    ...
    notification.show();
    console.log(`Meeting detection notification shown for ${platformName}`);
...
// Save as PDF handler
ipcMain.handle('saveAsPDF', async (event, data) => {
  try {
    const { title, content, isTranscript } = data;
    
    // Create a filename
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, -5);
    const suffix = isTranscript ? 'transcript' : 'summary';
    const defaultFileName = `${title}_${suffix}_${timestamp}.pdf`;
    ...
    await PDFGenerator.generatePDF(title, content, result.filePath, isTranscript);
...
ipcMain.handle('initializeGoogleCalendar', async () => {
  try {
...
ipcMain.handle('getGoogleAuthUrl', async () => {
  try {
    await googleCalendar.initialize();
    const result = await googleCalendar.getAuthUrlWithServer();
    if (result.success) {
...
ipcMain.handle('authorizeGoogleCalendar', async (event, code) => {
  try {
    const authorized = await googleCalendar.authorize(code);
...
ipcMain.handle('getUpcomingMeetings', async (event, maxResults = 10) => {
  try {
    const meetings = await googleCalendar.getUpcomingMeetings(maxResults);
...
ipcMain.handle('isGoogleCalendarAuthenticated', async () => {
  try {
    await googleCalendar.initialize();
...
ipcMain.handle('revokeGoogleCalendarAuth', async () => {
  try {
    const revoked = await googleCalendar.revokeAuth();
    return { success: revoked };
  } catch (error) {
    console.error('Error revoking Google Calendar auth:', error);
    return { success: false, error: error.message };
  }
});

// Chat with OpenAI about meeting notes
ipcMain.handle('chatWithOpenAI', async (event, userMessage, noteContent) => {
  try {
    // Check if we have an API key
    const apiKey = process.env.OPEN_API_KEY;
    if (!apiKey) {
      throw new Error('OpenAI API key not found. Please set OPEN_API_KEY in your .env file.');
    }
    ...
    // Create a system prompt that includes the note content and instructions
    const systemPrompt = `You are Rhythms, an AI assistant that helps users understand and work with their meeting notes. 

IMPORTANT RULES:
1. Only answer questions based on the information provided in the meeting notes below
2. If the user asks about something not mentioned in the meeting notes, respond with: "This is beyond the scope of the meeting notes. I can only help you with information that was discussed in the meeting."
3. Be helpful and concise in your responses
4. Focus on extracting actionable insights from the meeting content

MEETING NOTES:
${noteContent}

Please respond to the user's question based only on the information in these meeting notes.`;
